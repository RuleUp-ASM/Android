 ...
 *
 * 게이트에서 죽은 이벤트는 `ObsEvent` 가 만들어지기도 전에 사라져 **어디에도 흔적이 없다.**
 * 거부를 한 건도 관측하지 못한 상태에서 원인을 알 수 있어야 하므로, 설정 스냅샷을 그대로 노출한다.
 *
 * 싱크 내부 임계값·절단 규칙도 마찬가지다 — 정책 설정에는 안 잡히는 "안 찍히는 이유"라서
 * 여기 모아 한 화면에서 보게 한다.
 */
@Singleton
class ObservabilityDiagnostics
    ...
    constructor(
        ...
        /**
         * 비정상 신호 요약. **0 이면 빈 문자열**이다.
         *
         * 절단·싱크 실패는 평소 0 이라 항상 띄우면 잡음이 되지만, 0 이 아닌 순간에는
         * *"수집이 조용히 망가진 상태"* 를 알리는 유일한 신호다.
         */
