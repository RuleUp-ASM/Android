...
`data class` + **비었을 때의 뜻이 드러나게** 만든다. 타입 이름과 필드명만으로는 "null 이 0인지
모름인지"가 안 드러나고, 그걸 모르면 화면이 0%를 그린다.

먼저 이름·타입으로 미는 것이 맞는지 본다 — 틀렸을 때 잘못된 사실이 사용자에게 보이면 타입으로
가두고, 표기가 빠지는 정도면 주석 한 줄로 족하다. 판단 기준과 형식은 **comments 스킬**이 갖는다.
필드마다 기계적으로 주석을 붙이지 않는다.
...
- KDoc 첫 줄에 **근거**를 적는다 — 명세 경로(`명세 GET /challenges/{id}/room`)나 정책 이름.
  이 파일 안에 없는 사실이라 코드가 못 말한다(comments 스킬 ②).
...
상수 이름이 상황을 다 말하지 못할 때만 위에 한 줄 붙인다(`// 귀속일은 끝났고 확정 전 — 유예 구간`).
이름으로 되면 이름을 고친다 — comments 스킬.
...
- 테스트 이름은 **백틱 안 한국어 서술문**. 주석은 comments 스킬 ③ — 결과가 아니라 이게 깨지면
  무엇이 망가지는지.
