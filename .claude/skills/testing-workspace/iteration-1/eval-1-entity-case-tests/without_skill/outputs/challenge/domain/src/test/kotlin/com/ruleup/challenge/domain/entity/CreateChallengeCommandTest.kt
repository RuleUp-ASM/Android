...
class CreateChallengeCommandTest {
    ...
    @Test
    fun `주간 횟수 경계값 1 과 7 은 통과한다`() {
        // 실패 쪽만 보면 범위를 `1 until 7` 로 잘못 좁혀도 아무도 모른다.
    ...
    fun `그룹 정원이 1~10,000 을 벗어나면 만들 수 없다`() {
        ...
        assertEquals(ChallengeLimits.CAPACITY_MIN, group.copy(capacity = 1).capacity)
        assertEquals(ChallengeLimits.CAPACITY_MAX, group.copy(capacity = 10_000).capacity)
    }
