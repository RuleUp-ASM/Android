...
class InviteCodeStore
    ...
    constructor(
        ...
        suspend fun get(challengeId: String): String? {
            ...
            if (cached != null && System.currentTimeMillis() - cached.issuedAt < CACHE_TTL_MILLIS) {
                return cached.code
            }
            ...
            val response =
                try {
                    ...
                    // 실패해도 캐시를 비우지 않는다 — 비우면 비행기 모드에서 이미 받아둔 코드까지 못 쓴다.
                    throw e
                }
        ...
        /** 로그아웃에서만 부른다. 방 나가기에서 부르면 다른 방 코드까지 날아가 재발급을 탄다. */
        fun clear() {
            cache.clear()
        }
