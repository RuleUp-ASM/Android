 ...
 * 그 측정의 구간 하나.
 *
 * **구간을 열로 펴지 않고 행으로 둔다** — [TtiTimeline] 이 늘어도 스키마가 그대로다.
...
internal data class TtiSpanEntity(
    val recordId: String,
    /** [TtiTimeline] 의 이름. enum 서수를 넣지 않는 것은 순서가 바뀌어도 DB 가 깨지지 않게 하려는 것이다. */
    val timeline: String,
    val startedAt: Long,
    val endedAt: Long?,
)
 ...
 * 저장 모양 → 도메인 모양.
 *
 * 이름이 [TtiTimeline] 에 없는 구간(예전 버전이 남긴 것)은 버린다. 그 기록은 구간 수가 모자라
 * 완성으로 잡히지 않고, 오래되면 정리된다.
 */
