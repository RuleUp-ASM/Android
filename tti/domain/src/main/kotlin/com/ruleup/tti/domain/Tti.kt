 ...
 * 측정 한 건을 가리키는 손잡이.
 *
 * 같은 화면이라도 백스택에 두 번 쌓이면 서로 다른 측정이므로 [id] 로 가른다. 화면은 이 값을
 * `remember` 로 들고 다니고, 구간의 원본은 [TtiRecordStore] 에 있다.
 */
data class Tti(
...
data class TtiRecord(
     ...
     *
     * **처음과 끝의 차이가 아니라 합인 이유**: 구간 사이에는 측정하지 않는 빈 시간(사용자 입력
     * 대기 등)이 끼어들 수 있고, 그것까지 TTI 로 세면 화면이 느려진 것처럼 보인다.
     */
