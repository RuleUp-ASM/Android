...
/**
 * 화면별 TTI 기록기. 앱 전체에 하나만 있고 `:app` 이 [init] 으로 연다.
 *
...
interface TtiRecorder {
     ...
     *
     * 아직 완성되지 않았으면(예: 이미지가 아직 내려오는 중) 쏘지 않고 그대로 둔다. 그 기록은
     * [destroy] 나 다음 [init] 때 완성돼 있으면 그때 나간다.
     */
    fun shot(
        tti: Tti,
        pageName: String,
    ...
    /**
     * 기록을 닫는다. 앱이 백그라운드로 내려가는 마지막 신호에서 부른다.
     *
     * 저장소에 남은 완성 기록을 모두 쏘고 나서 스코프를 정리한다. 안드로이드는 프로세스 종료를
     * 알려주지 않으므로, 못 쏘고 죽은 것은 다음 [init] 이 주워 간다.
     */
    fun destroy()
}
 ...
 * 기본 구현을 만든다.
 *
 * 구현 클래스를 내보내지 않는 것은 바깥이 [TtiRecorder] 계약만 보게 하려는 것이다. 디스패처는
 * 인자로 받는다 — 이 모듈은 순수 JVM 이라 Android 의 IO 디스패처 선택을 여기서 하지 않는다.
 */
