 ...
 * 자식 하나가 실패해도 기록기 전체가 멈추면 안 되므로 스코프는 [SupervisorJob] 을 쓰고, 각 작업은
 * `runCatching` 으로 감싼다 — **계측 실패가 앱을 죽이는 일은 없어야 한다.**
 */
...
internal class TtiRecorderImpl(
     ...
     * (Room 의 suspend DAO 가 그렇다) 먼저 시작한 작업이 첫 호출에서 멈춘 사이 다음 작업이 앞질러
     * 간다. **시작 순서(디스패처)와 처리 순서(이 잠금)를 함께 묶어야 순서가 선다.**
     *
     * 순서가 어긋나면 측정이 조용히 사라진다. end 가 start 보다 먼저 처리되면 아직 없는 행을
     * 갱신하는 셈이라 그 구간은 영영 닫히지 않고, 쏘기와 삭제 사이에 다른 쏘기가 끼어들면 같은
     * 기록이 두 번 나간다.
     */
    ...
    override fun init() {
        ...
        // 지난 실행이 쏘지 못하고 죽은 것을 먼저 흘려보낸다. 안드로이드는 프로세스 종료를 알려주지
        // 않으므로 "완성됐지만 아직 안 쏜 기록"을 실제로 회수하는 곳은 대부분 여기다.
        record("init") {
            shootCompleted()
    ...
    override fun shot(
        ...
        record("shot $pageName") {
            // 앞선 쏘기가 이미 보내고 지웠으면 여기서는 없는 기록이 된다 — 화면이 다 그려져 쏘는 것과
            // 마지막 구간이 닫혀 쏘는 것이 겹칠 수 있다.
    ...
    /**
     * 쏘고 나서 지운다. 쏘기가 실패하면 예외가 그대로 올라가 삭제까지 가지 않는다 — 다음 기회에
     * 다시 나간다.
     */
     ...
     * 계측이 실패했다는 사실만 남긴다.
     *
     * 플랫폼 로거를 쓰지 않는 것은 이 모듈이 순수 JVM 이기 때문이다. 안드로이드에서는 표준 출력도
     * Logcat 으로 들어온다.
     */
    private fun warn(
        what: String,
