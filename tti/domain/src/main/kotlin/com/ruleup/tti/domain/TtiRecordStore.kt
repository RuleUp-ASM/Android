 ...
 * 측정 기록 저장 포트.
 *
 * 프로세스가 죽어도 남아 있어야 하므로 메모리가 아니라 영속 저장소를 전제로 한다. 어디에 어떻게
 * 담는지는 `:tti:data` 가 정한다.
 *
...
interface TtiRecordStore {
     ...
     * 기록과 그 구간을 연다.
     *
     * 기록이 이미 있으면 [createdAt] 을 덮어쓰지 않고, 구간이 이미 열려 있으면 [startedAt] 도
     * 지킨다 — 컴포지션이 다시 돌아 start 가 또 와도 처음 시각이 기준이어야 하기 때문이다.
     */
