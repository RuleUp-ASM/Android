 ...
 *
 * 기록하는 일과 쏘는 일을 나눠 둔다 — [record] 는 화면·사용자·시각만 그 자리에서 찍어 넘기고 곧바로
 * 돌아간다(그래야 기록하는 화면이 전송을 기다리지 않는다). 전송만 IO 스코프에서 돈다.
 *
...
internal class BizLoggerImpl(
     ...
     * 먼저 얹힌 건이 첫 중단 지점에서 멈춘 사이 다음 건이 앞질러 나간다. 시작 순서(디스패처)와
     * 나가는 순서(이 잠금)를 함께 묶어야 일어난 순서대로 도착한다.
     *
    ...
    override fun record(event: BizEvent) {
        // 화면·사용자는 일어난 시점의 값이어야 한다 — 전송이 도는 사이 화면이 바뀌거나 로그아웃될 수 있다.
        shot(
            BizLog(
                event = event,
     ...
     *
     * 밖으로 열지 않는 것은 기록과 전송이 언제나 한 짝이기 때문이다 — 쌓아 두는 곳이 없으니
     * 따로 쏠 것도 없다. 바깥이 보는 손잡이는 [record] 하나다.
     */
    private fun shot(log: BizLog) {
        // init 전이거나 destroy 뒤면 보낼 곳이 없다. 로그가 없다고 화면이 멈출 이유는 없으므로 조용히 버린다.
     ...
     * 로그가 실패했다는 사실만 남긴다.
     *
     * 플랫폼 로거를 쓰지 않는 것은 이 모듈이 순수 JVM 이기 때문이다.
