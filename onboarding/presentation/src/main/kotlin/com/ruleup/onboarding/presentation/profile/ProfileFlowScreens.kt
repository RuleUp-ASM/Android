...
/**
 * 프로필 설정 플로우의 페이지별 화면. 6개 화면이 Activity 스코프의 단일 [ProfileViewModel] 을
 * 공유하므로 입력값이 페이지 이동에도 누적된다. 단순 전진/후진은 각 Content 가 LocalNavigationHelper 로
 * 직접 처리하고, 비동기 분기(닉네임 검사·가입 제출)는 ViewModel 이 처리한다.
 */
@Composable
private fun sharedProfileViewModel(): ProfileViewModel = hiltViewModel(viewModelStoreOwner = rememberActivityViewModelStoreOwner())
...
/** 06 · 약관 동의. "시작하기" 는 가입을 제출하고 성공 시 ViewModel 이 홈으로 이동시킨다. */
@Composable
