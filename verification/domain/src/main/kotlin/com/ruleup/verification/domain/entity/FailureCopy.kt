...
/**
 * 서버가 준 사유 코드의 문구. 모르는 코드는 [FailureReason.UNKNOWN] 문구로 떨어진다.
 *
 * 문자열로 들고 있는 화면(활동 캘린더)이 같은 표를 보게 하려고 둔다 — 화면마다 표를 따로 두면
 * 한쪽만 늘어나, 같은 실패가 방에서는 "머문 시간이 모자랐어요", 캘린더에서는 그냥 "실패"가 된다(VER-04 · VER-05).
 */
